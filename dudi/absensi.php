...
$page_title = "Rekap Absensi PKL Murid";
...
$where_clauses = [
    "sp.tempat_pkl_id = $tempat_pkl_id",
    "sp.tahun_pelajaran_id = $active_tahun_id",
    "sp.status = 'aktif'"
];

if (!empty($search)) {
    $where_clauses[] = "(s.nama_siswa LIKE '%$search%' OR s.nis LIKE '%$search%' OR k.nama_kelas LIKE '%$search%')";
}
...
$pagin = get_pagination_data($conn, "absensi_harian ah JOIN siswa s ON ah.siswa_id = s.id JOIN siswa_pkl sp ON s.id = sp.siswa_id JOIN siswa_kelas sk ON s.id = sk.siswa_id AND sk.tahun_pelajaran_id = sp.tahun_pelajaran_id JOIN kelas k ON sk.kelas_id = k.id", 15, $where_sql);

$q_abs = "SELECT ah.*, s.nama_siswa, s.nis, k.nama_kelas
...
<div class="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto pb-24">
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
            <h1 class="text-3xl font-black text-slate-800 tracking-tight italic">Rekap Absensi PKL Murid</h1>
            <p class="text-slate-500 font-medium">Monitoring kehadiran presensi lokasi GPS siswa PKL di industri Anda.</p>
        </div>
    </div>
    ...
    <div class="lux-card p-6 mb-8 bg-gradient-to-br from-indigo-50/50 to-white">
        <form action="" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div class="space-y-1">
                ...
                <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Nama, NIS, kelas..." class="w-full px-4 py-2.5 rounded-xl border border-slate-200 outline-none focus:ring-4 focus:ring-indigo-100 bg-white text-xs font-semibold">
            </div>
            <div class="space-y-1">
            ...
            <div class="flex items-end gap-2">
                ...
                <a href="<?= BASE_URL ?>dudi/absensi.php" class="px-4 py-2.5 bg-slate-200 text-slate-600 font-bold rounded-xl text-xs hover:bg-slate-300 transition-all">Reset</a>
            </div>
        </form>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-center text-slate-400 font-bold italic">Belum ada catatan presensi GPS untuk siswa PKL Anda.</td>
                    </tr>
                    <?php else: ?>
