...
if ($kelas_id > 0) {
    ...
    // Filter by guru_id for teacher role, or allow all for admin/waka
    $where_guru = in_array($_SESSION['role'], ['admin', 'waka']) ? "1=1" : "j.guru_id = $guru_id";
    $where_mapel = ($mapel_id > 0) ? " AND j.mapel_id = $mapel_id" : "";

    // 1. Fetch all journals submitted by this teacher for selected class & date range
    ...
    // 2. Fetch list of students in the selected class
    ...
    // Fallback if active_tahun_id filter yields no rows
