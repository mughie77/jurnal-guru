...
$page_title = "Rekap Buku Kejadian Saya";
...
$pagin = get_pagination_data($conn, "buku_kejadian bk JOIN kelas k ON bk.kelas_id = k.id", 15, $where_sql);

$query = "SELECT bk.*, k.nama_kelas, mp.nama_mapel
...
<div class="max-w-7xl mx-auto pb-20 px-4">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
        <div>
            ...
            <p class="text-slate-500 font-medium">Catatan riwayat kejadian dan pembinaan siswa selama kegiatan pembelajaran Anda.</p>
        </div>
        <div class="flex gap-3">
            <a href="export_buku_kejadian_excel.php?<?= http_build_query($_GET) ?>" class="px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold transition-all shadow-lg shadow-emerald-100 flex items-center text-xs">
    ...
    <div class="lux-card p-6 mb-8 bg-gradient-to-br from-amber-50/50 to-white border border-amber-100/50">
        <form action="" method="GET" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            ...
            <div>
                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2">Cari Siswa / Kejadian</label>
                <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Nama siswa, uraian, tindak lanjut..." class="w-full px-4 py-2.5 rounded-xl border border-slate-200 outline-none focus:ring-4 focus:ring-amber-100 bg-white font-semibold text-xs text-slate-700">
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-slate-400 font-bold italic">Belum ada catatan buku kejadian terdaftar.</td>
                    </tr>
                    <?php else: ?>
