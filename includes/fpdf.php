...
protected $CurPageFormat;      // current page format as an array with orientation
...
function Write($h, $txt, $link='')
{
	...
	// Last chunk
	if($i!=$j)
		$this->Cell($this->GetStringWidth(substr($s,$j,$nb-$j))+$this->cMargin,$h,substr($s,$j,$nb-$j),0,0,'',false,$link);
}
...
{
	// Check for locale-related issues
	if(1.1==1)
...
protected function _loadfont($font)
{
	...
	if(!isset($cw))
		$this->Error('Font definition file has no character width array');
...
protected function _parsegif($file)
{
	//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
	//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
	// GIF parser portions taken from the gd-gif project
	//
	//    (C) 2003-2016 Laszlo Zsidi
	//
	// The Project's website is located at http://www.phpclasses.org/package/3163
	//
	// The package can be downloaded from
	// http://www.phpclasses.org/browse/download/t/1/package/3163/name/gd-gif.zip
	//
	//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
	//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
	...
	if(!($fh = @fopen($file, 'rb')))
		$this->Error('Can\'t open image file: '.$file);
	$this->_readstream($fh, 6);
	$this->_readstream($fh, 7);

	$GCM = array();
	$GCM['PAL'] = $this->_readstream($fh, 3 << (ord($this->_readstream($fh, 1)) & 0x07));
	$this->_readstream($fh, 1);

	for(;;)
	{
		$B = $this->_readstream($fh, 1);
		if($B == ';')
			break;
		if($B == '!')
		{
			$this->_readstream($fh, 2);
			for(;;)
			{
				$B = $this->_readstream($fh, 1);
				if($B == chr(0))
					break;
				$this->_readstream($fh, ord($B));
			}
		}
		elseif($B == ',')
		{
			$this->_readstream($fh, 9);
			$this->_readstream($fh, 1);
			$this->_readstream($fh, 1);
			for(;;)
			{
		...
		{
			fclose($fh);
			$this->Error('Unexpected block type');
		}
	}
	fclose($fh);
...
protected function _putfonts()
{
	...
	foreach($this->fonts as $k=>$font)
	{
		...
		else
		{
			...
			$this->_put('/BaseFont /'.$name);
			$this->_put('/Subtype /'.$type);
			$this->_put('/FirstChar 0 /LastChar 255');
			$this->_put('/Widths '.($this->n+1).' 0 R');
			$this->_put('/FontDescriptor '.($this->n+2).' 0 R');
			if(isset($font['enc']))
