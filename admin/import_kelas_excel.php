...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['excel_file'])) {
    if ($xlsx = SimpleXLSX::parse($_FILES['excel_file']['tmp_name'])) {
        ...
        try {
            $stmt = mysqli_prepare($conn, "INSERT IGNORE INTO kelas (nama_kelas) VALUES (?)");
            foreach ($rows as $row) {
            ...
            mysqli_commit($conn);
            $message = "Berhasil mengimpor $count kelas baru."; $message_type = 'success';
...
<div class="mb-8">
    <h1 class="text-3xl font-bold text-slate-800 tracking-tight italic">Import Kelas</h1>
    <p class="text-slate-500">Tambah daftar kelas secara massal dari Excel.</p>
</div>
...
<div class="lux-card p-8">
    <div class="mb-8 p-6 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center justify-between">
        <div>
            ...
            <p class="text-indigo-700/70 text-sm italic">Hanya kolom 'Nama Kelas'</p>
        </div>
        <a href="<?= BASE_URL ?>api/download_template.php?type=kelas" class="inline-flex items-center px-5 py-2.5 bg-white text-indigo-600 font-bold rounded-xl border border-indigo-100 shadow-sm hover:shadow-md transition-all">
    ...
    <form action="" method="POST" enctype="multipart/form-data" class="space-y-6">
        <div class="relative group">
            ...
            <label for="excel_file" class="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-slate-300 rounded-3xl bg-slate-50 group-hover:bg-indigo-50 transition-all cursor-pointer">
                <i class="fa fa-school text-4xl text-slate-400 group-hover:text-indigo-500 mb-4 transition-colors"></i>
                <p class="text-sm text-slate-500"><span class="font-bold">Pilih file Excel</span> untuk daftar kelas</p>
                <p id="fileName" class="mt-4 text-indigo-600 font-bold"></p>
            </label>
        </div>
        <div class="flex gap-4">
            <a href="kelas.php" class="flex-1 px-6 py-3.5 rounded-xl border border-slate-200 text-slate-600 font-bold text-center">Kembali</a>
            <button type="submit" class="flex-[2] bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3.5 rounded-xl shadow-lg transition-all">Import Kelas</button>
        </div>
    </form>
</div>
