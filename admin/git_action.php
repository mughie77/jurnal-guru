...
switch ($action) {
    case 'status':
        if (!$is_git_repo) {
            $output_str = "Peringatan: Folder .git tidak terdeteksi di server ini (Sistem belum diinisialisasi sebagai Repositori Git).\n\nAnda dapat mengklik tombol \"Set Remote origin URL\" di panel kiri untuk menginisialisasi repository Git ini secara otomatis.";
            $res = [
                'success' => false,
                'message' => 'Repositori Git belum diinisialisasi.',
                'log' => $output_str
            ];
        } else {
    ...
    case 'init_origin':
        ...
        $init_logs[] = "> git remote config origin";
        $init_logs[] = implode("\n", $output);
    ...
    case 'pull':
        if (!$is_git_repo) {
            $res = [
                'success' => false,
                'message' => 'Repositori Git belum diinisialisasi. Silakan klik tombol "Set Remote origin URL" terlebih dahulu.',
